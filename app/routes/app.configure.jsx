...
export const action = async ({ request }) => {
  ...
  const productIds = formData.get("productIds").split(",");
  const zoneX = parseFloat(formData.get("zoneX")) || 0;
  const zoneY = parseFloat(formData.get("zoneY")) || 0;
  const zoneWidth = parseFloat(formData.get("zoneWidth")) || 100;
  const zoneHeight = parseFloat(formData.get("zoneHeight")) || 50;
  const zoneAngle = parseFloat(formData.get("zoneAngle")) || 0;
...
export default function Configure() {
  const { product, config, globalSettings, allIds } = useLoaderData();
    ...
    >
      <TitleBar title={`Configure Zone for ${product.title}`}>
        <button variant="primary" onClick={handleSave}>
          Save{" "}
          {allIds.length > 1 ? `(${allIds.length} Products)` : "Configuration"}
        </button>
      </TitleBar>
      ...
      <BlockStack gap="500">
        {allIds.length > 1 && (
          <Banner title="Bulk Configuration Mode" tone="info">
            You are configuring {allIds.length} products simultaneously. The
            changes you save here will be applied to all selected products
            instantly.
          </Banner>
        )}
            ...
            <Card>
              <BlockStack gap="400">
                <Text as="h2" variant="headingMd">
                  Embroidery Zone Preview
                </Text>
                <Text as="p" tone="subdued">
                  Drag the sliders to adjust the blue box. This determines
                  exactly where the customer's text will be placed on the final
                  image.
                </Text>
            ...
            <BlockStack gap="500">
              ...
              <Card>
                <BlockStack gap="300">
                  ...
                  <Text as="p" tone="subdued">
                    Fonts, Colors, and Pricing are applied globally across all
                    products.
                  </Text>
                  <List type="bullet">
                    <List.Item>
                      2 Lines: £{globalSettings?.price2Lines}
                    </List.Item>
                    <List.Item>
                      3 Lines: £{globalSettings?.price3Lines}
                    </List.Item>
                    <List.Item>
                      Image Upload: £{globalSettings?.priceImage}
                    </List.Item>
                  </List>
