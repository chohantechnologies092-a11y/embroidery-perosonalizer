...
import { useLoaderData, useSubmit, useActionData, useNavigate, useNavigation, useRouteError, isRouteErrorResponse } from "react-router";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === "setup_addon") {
    ...
    const price2 = parseFloat(formData.get("price2Lines") as string) || 5;
    const price3 = parseFloat(formData.get("price3Lines") as string) || 6;
    ...
    frameSizes.forEach((size: any) => {
      ...
      variants.push({ optionValues: [{ optionName: "Frame Size", name: `${size.name} (1 Line)` }], price: basePrice.toFixed(2) });
      variants.push({ optionValues: [{ optionName: "Frame Size", name: `${size.name} (2 Lines)` }], price: (basePrice + price2).toFixed(2) });
      variants.push({ optionValues: [{ optionName: "Frame Size", name: `${size.name} (3 Lines)` }], price: (basePrice + price3).toFixed(2) });
    });
    ...
    `, {
      variables: {
        synchronous: true,
        input: {
          ...(existingSettings?.addonProductId ? { id: existingSettings.addonProductId } : {}),
          ...
          variants: [...variants, { optionValues: [{ optionName: "Frame Size", name: "Image Upload" }], price: parseFloat(formData.get("priceImage") as string || "3").toFixed(2) }]
        }
      }
    });
    ...
    if (newProduct) {
      ...
      const addonVariants = newProduct.variants.edges.map((e: any) => ({
      ...
      const defaultColors = formData.get("defaultColors") as string || "[]";
      ...
      return { success: true, message: "Add-on product created and synced to Storefront!" };
    }
    return { success: false, message: "Failed to create add-on product." };
  }

  // Normal Save
  const price2Lines = parseFloat(formData.get("price2Lines") as string) || 0;
  const price3Lines = parseFloat(formData.get("price3Lines") as string) || 0;
  const priceImage = parseFloat(formData.get("priceImage") as string) || 0;
  
  const defaultFonts = formData.get("defaultFonts") as string;
  const defaultColors = formData.get("defaultColors") as string;
  const frameSizes = formData.get("frameSizes") as string;

  await prisma.globalSettings.upsert({
    where: { shop: session.shop },
    update: { price2Lines, price3Lines, priceImage, defaultFonts, defaultColors, frameSizes },
    create: { shop: session.shop, price2Lines, price3Lines, priceImage, defaultFonts, defaultColors, frameSizes }
  });
  ...
  const existingMetafieldQuery = await admin.graphql(`{ shop { metafield(namespace: "embroidery_app", key: "settings") { value } } }`);
  const existingMetafieldData = await existingMetafieldQuery.json();
  let addonVariants = [];
  try {
    const existingVal = JSON.parse(existingMetafieldData.data.shop.metafield?.value || "{}");
...
export default function Settings() {
  ...
  const [price2Lines, setPrice2Lines] = useState(settings.price2Lines.toString());
  const [price3Lines, setPrice3Lines] = useState(settings.price3Lines.toString());
  ...
  const [colors, setColors] = useState(() => {
    try {
      ...
      if (typeof parsed[0] === 'string') {
        return parsed.map((hex: string) => ({ name: hex, hex: hex }));
      }
      return parsed;
    } catch {
      return [{ name: "Black", hex: "#000000" }, { name: "White", hex: "#FFFFFF" }];
    }
  });
  ...
  const handleSave = () => {
    ...
    const fontArray = fonts.split(",").map((f: string) => f.trim()).filter(Boolean);
      ...
      <Layout>
        <Layout.Section>
          <BlockStack gap="500">
            <Card>
              <BlockStack gap="400">
                <Text as="h2" variant="headingMd">Dynamic Frame Sizes</Text>
                <Text as="p" tone="subdued">
                  Add the frame sizes available for your products, and set the base price for 1 line of text.
                </Text>
                
                <BlockStack gap="300">
                  {frameSizes.map((size: any, index: number) => (
            ...
            <Card>
              <BlockStack gap="400">
                <Text as="h2" variant="headingMd">Extra Line & Image Pricing</Text>
                <Text as="p" tone="subdued">
                  Set the extra charges applied when a customer selects more than 1 line or adds an image.
                </Text>
            ...
            <Card background="bg-surface-secondary">
              <BlockStack gap="300">
                <Text as="h2" variant="headingMd">Dynamic Pricing Sync</Text>
                <Text as="p">
                  Click the button below to automatically update the Add-on Product in your Shopify store with the Frame Sizes defined above.
                </Text>
                {settings.addonProductId && (
                  <Banner tone="success">
                    Add-on Product is configured. Click below if you changed the sizes.
                  </Banner>
                )}
                <InlineStack>
                  <Button variant="primary" onClick={handleCreateAddon} loading={navigation.state === "submitting"}>
        ...
        <Layout.Section variant="oneThird">
          <BlockStack gap="500">
            <Card>
              <BlockStack gap="400">
                <Text as="h2" variant="headingMd">Enhanced Color Picker</Text>
                <Text as="p" tone="subdued">Define thread colors visually so customers see the exact shade and name.</Text>
                ...
                {colors.map((color: any, index: number) => (
                  <InlineStack key={index} gap="200" blockAlign="center" wrap={false}>
                    <input 
                      ...
                      style={{ width: '40px', height: '40px', padding: '0', border: 'none', cursor: 'pointer', borderRadius: '50%' }}
                    />
                    <div style={{ flex: 1 }}>
                      <TextField
                        labelHidden
                ...
                <InlineStack>
                  <Button onClick={() => setColors([...colors, { name: "New Color", hex: "#000000" }])}>
                    + Add Color
                  </Button>
                </InlineStack>
              </BlockStack>
            ...
            <Card>
              <BlockStack gap="400">
                <Text as="h2" variant="headingMd">Fonts</Text>
                <TextField
